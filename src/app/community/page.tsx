...
export default function CommunityPage() {
  ...
  const subscribe = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    setStatus('sending'); setMsg('Signing up…')
    const form = new FormData(e.currentTarget)
    const data = Object.fromEntries(form.entries())
    try {
      const res = await fetch('/api/community/subscribe', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) })
      const j = await res.json()
      if (!res.ok || !j.ok) throw new Error(j?.error || 'Failed')
      setStatus('ok'); setMsg('Thanks! You’re on the list.')
      ...
      <section className="mx-auto max-w-6xl px-4 pb-16">
        <div className="glass rounded-2xl p-6">
          ...
          <ul className="list-disc list-inside text-white/80 mt-2">
            <li>Templates & prompt libraries</li>
            <li>Monthly community build sessions (online)</li>
            <li>Early access to seminars & discounts</li>

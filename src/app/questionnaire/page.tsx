...
function QuestionnaireForm() {
  ...
  // Move focus to status updates (accessibility)
  useEffect(() => {
  ...
  const submit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Honeypot: if bots fill this, we silently succeed but drop the payload server-side if desired
    ...
    const payload: Record<string, string> = {
      ...
      // Include a hint for server to ignore known-bot submissions
      _honeypot: hp,
    }

    // Client-side guard for required fields (in addition to HTML required)
    ...
    try {
      ...
      setStatus('ok')
      setMsg('Thanks! Our team will email/WhatsApp you shortly with a tailored plan.')
      e.currentTarget.reset()
    } catch (err: any) {
      const aborted = err?.name === 'AbortError'
      setStatus('error')
      setMsg(aborted ? 'Network is slow—please try again.' : (err?.message || 'Something went wrong'))
    }
  }
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-12">
      ...
      <p className="text-white/70 mt-2">
        Tell us about your goals. We’ll follow up with a custom plan.
      </p>
      ...
      <p
        ...
        className={`mt-4 text-sm ${
          status === 'ok' ? 'text-green-300' : status === 'error' ? 'text-red-300' : 'text-white/70'
        }`}
        role="status"
        aria-live="polite"
        tabIndex={-1}
      ...
      <form onSubmit={submit} className="mt-6 grid gap-4">
        ...
        {/* If user arrived from a course page, pre-fill it (read-only to avoid confusion) */}
        {prefillCourse ? (
          <div>
            ...
            <input
              ...
              readOnly
              className="bg-white/10 rounded px-3 py-2 w-full opacity-80"
            />
          </div>
        ) : (
          <div>
            ...
            <input
              name="course"
              placeholder="If you have a course in mind"
        ...
        <button
          ...
          className="rounded-lg border border-white/20 px-4 py-2 hover:bg-white/10 disabled:opacity-60"
        >
          {status === 'sending' ? 'Submitting…' : 'Submit'}
        </button>

        <p className="text-xs text-white/60">
          Pricing is shared after we review your responses.
        </p>
      </form>
    </main>
  )

...
export default function ProgramFinder() {
  ...
  async function onSubmit() {
    ...
    try {
      ...
      void track('program_finder_result', {
        ...
        title: data?.title,
        price: data?.price,
        automatedBy: data?.automatedBy || 'rules',
      });
    } catch {
      const data: Result = {
        ...
        why: [
          ...
          'Repeatable: you leave with templates and prompts.',
        ],
        path: '/programs',
        price: 'R1299',
      ...
      setResult(data);
      void track('program_finder_result_fallback', {
        role,
        goal,
        time,
        budget,
        title: data.title,
        automatedBy: data.automatedBy,
      });
    } finally {
      setLoading(false);
      setStep(5);
  ...
  function selectAndTrack<T extends string>(setter: (value: T) => void, field: string, value: T) {
    setter(value);
      ...
      <div className="glass mt-8 rounded-2xl p-6 md:p-8">
        {step === 1 && (
          <Step
            ...
            value={role}
            onChange={(v) => selectAndTrack(setRole, 'role', v as typeof role)}
            onNext={() => {
              setStep(2);
              void track('program_finder_next', { step: 1 });
            }}
            canNext={canNext1}
          />
        )}
        ...
        {step === 2 && (
          <Step
            ...
            options={[
              ...
              ['customers', 'Leads and Customer Service'],
              ['ops', 'Operations and Reports'],
              ['learning', 'Learn the Fundamentals'],
            ]}
            value={goal}
            onChange={(v) => selectAndTrack(setGoal, 'goal', v as typeof goal)}
            onBack={() => setStep(1)}
            onNext={() => {
              setStep(3);
              void track('program_finder_next', { step: 2 });
            }}
            canNext={canNext2}
          />
        )}
        ...
        {step === 3 && (
          <Step
            ...
            value={time}
            onChange={(v) => selectAndTrack(setTime, 'time', v as typeof time)}
            onBack={() => setStep(2)}
            onNext={() => {
              setStep(4);
              void track('program_finder_next', { step: 3 });
            }}
            canNext={canNext3}
          />
        )}
        ...
        {step === 4 && (
          <Step
            ...
            options={[
              ['free', 'Free (Clinic)'],
              ['under1500', 'Approx R1299'],
              ['custom', 'Custom (Team / Org)'],
            ]}
            value={budget}
            onChange={(v) => selectAndTrack(setBudget, 'budget', v as typeof budget)}
            onBack={() => setStep(3)}
            onNext={onSubmit}
            canNext={canNext4}
        ...
        {step === 5 && result && (
          <ResultCard
            ...
            onRestart={() => {
              setStep(1);
              setResult(null);
              void track('program_finder_restart', {});
            }}
          />
        )}
      </div>
  ...
  return (
    <div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        <div className="glass rounded-xl p-4 text-white/85 text-sm">You will leave with a working workflow</div>
        <div className="glass rounded-xl p-4 text-white/85 text-sm">Includes templates and prompt library</div>
        <div className="glass rounded-xl p-4 text-white/85 text-sm">Privacy-first guidance for your data</div>
      </div>
    </div>
  );
}
